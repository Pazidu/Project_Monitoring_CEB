...
export const EditReportModal = ({
  ...
  // Populate state when modal opens with selected report data
  useEffect(() => {
    if (visible && reportData) {
      setReportName(
        reportData.title || reportData.name || "Maintenance and Support Cost",
      );
      setReportTitle(
        reportData.reportTitleCustom ||
      ...
      if (
        reportData.includedSections &&
        Array.isArray(reportData.includedSections)
      ) {
        setSections(
          INITIAL_SECTIONS.map((sec) => ({
            ...sec,
            selected: reportData.includedSections.includes(sec.id),
          })),
        );
      } else {
  ...
  const handleSaveSubmit = () => {
    ...
    const updatedReportPayload = {
      ...
      updatedDate: `Updated ${new Date().toLocaleDateString("en-GB", {
        day: "2-digit",
        month: "short",
        year: "numeric",
        ...
        <View style={styles.headerRow}>
          <View style={styles.titleSubtitleBox}>
            ...
            <Text variant="bodySmall" style={styles.headerSubtitle}>
              Tick the sections to include. Anyone with project access can
              download this report at any time.
            </Text>
          </View>
          <IconButton
            icon="close"
        ...
        >
          ...
          <View style={styles.sectionHeaderRow}>
            <Text style={styles.sectionsTitleText}>
              Sections to include ({selectedCount})
            </Text>
...
const styles = StyleSheet.create({
  ...
  textInputStyle: { fontSize: 13, padding: 6 },
  sectionHeaderRow: {
    flexDirection: "row",
    justifyContent: "space-between",
    alignItems: "center",
